...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Approved Loans</h1>
    <p class="text-gray-500 text-sm mt-1">All currently approved and active loans.</p>
</div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3 font-semibold text-gray-800">₱{{ number_format($loan->balance, 2) }}</td>
                    <td class="px-4 py-3 text-gray-600">₱{{ number_format($loan->loan_amount / 6, 2) }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $loan->due_date ? \Carbon\Carbon::parse($loan->due_date)->format('M d') : 'N/A' }}</td>
                    <td class="px-4 py-3">
                        @php 
                        ...
                        <span class="text-[10px] text-gray-500">{{ $pct }}% paid</span>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="px-4 py-8 text-center text-gray-500">No approved loans found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
