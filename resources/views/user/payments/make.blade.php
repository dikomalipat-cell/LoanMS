...
<div class="bg-primary rounded-xl shadow-sm border border-slate-700 p-6 max-w-2xl">
    <div class="bg-primary-light rounded-lg p-4 mb-6 border border-green-100 flex justify-between items-center">
        <div>
            <p class="text-sm text-gold font-medium">Current Amount Due</p>
            <p class="text-xl font-bold text-white">₱ {{ number_format($client?->balance / 6 ?? 0, 2) }}</p>
        </div>
        <div class="text-right">
            <p class="text-sm text-slate-300">Due Date</p>
            <p class="font-medium text-white">{{ $client?->due_date ? \Carbon\Carbon::parse($client->due_date)->format('M d, Y') : 'N/A' }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('user.payments.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <label class="border border-slate-700 rounded-lg p-4 flex items-center cursor-pointer hover:border-green-500 transition">
                    <input type="radio" name="payment_method" value="bank_transfer" class="text-gold focus:ring-green-500 mr-3" checked>
                    <div>
                ...
                <label class="border border-slate-700 rounded-lg p-4 flex items-center cursor-pointer hover:border-green-500 transition">
                    <input type="radio" name="payment_method" value="ewallet" class="text-gold focus:ring-green-500 mr-3">
                    <div>
                        <p class="font-medium text-white">E-Wallet</p>
                        <p class="text-xs text-slate-400">GCash, Maya</p>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-primary-dark transition cursor-pointer">
